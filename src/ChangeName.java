...
public class ChangeName {
    ...
    public ChangeName(Compte c, Compte[] comptes) {
        if (c == null) {
            throw new IllegalArgumentException("Le compte passé au constructeur est null.");
        }

        fenetre = new JFrame("SwiterBank - Changement de nom");
        ...
        bouton.addActionListener(e -> {
            String nouveauNom = inputId.getText();
            if (nouveauNom.isEmpty()) {
                JOptionPane.showMessageDialog(fenetre, "Le nom ne peut pas être vide.", "Erreur", JOptionPane.ERROR_MESSAGE);
            } else {
                c.declarationNom(nouveauNom);
                JOptionPane.showMessageDialog(fenetre, "Nom changé avec succès !");
